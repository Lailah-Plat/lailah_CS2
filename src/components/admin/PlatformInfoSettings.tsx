...
export default function PlatformInfoSettings({ 
  ...
  const tabs = [
    ...
    { id: 'gov_integrations', label: 'الربط الحكومي والتحقق', icon: ShieldCheck },
  ];
  ...
  return (
    <div className="relative">
      ...
      <div className="bg-white rounded-3xl border border-slate-200 p-6 md:p-8 shadow-sm">
        ...
        {/* Tab 7: Government Verification Sandbox & Integrations (Nafath, Wathq, Shamous) */}
        {activeTab === 'gov_integrations' && (
          <div className="space-y-8 animate-in fade-in duration-300 text-right" dir="rtl">
            <div className="border-b border-slate-150 pb-5">
              <h4 className="text-xl font-bold text-slate-800 flex items-center gap-2">
                <ShieldCheck className="w-5 h-5 text-amber-500" />
                إعدادات الربط والتحقق الحكومي الإلكتروني المباشر (Nafath / Wathq / Shamous)
              </h4>
              <p className="text-xs text-slate-500 mt-1">
                تهيئة وإدارة بروتوكولات الفحص الآلي المباشر للهويات والسجلات التجارية ونظام شموس الأمني للمنشآت والمرافق مع مفاتيح الربط وأزرار التفعيل التلقائي.
              </p>
            </div>
            ...
            <div className="bg-slate-50 border border-slate-200 rounded-2xl p-6 space-y-5">
              <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 border-b border-slate-200 pb-4">
                <div className="flex items-center gap-3">
                  <div className="w-10 h-10 rounded-xl bg-emerald-600 text-white font-black flex items-center justify-center text-xs shadow-sm">
                    نفاذ
                  </div>
                  <div>
                    <h5 className="font-extrabold text-slate-800 text-sm">نظام النفاذ الوطني الموحد (Nafath IAM API)</h5>
                    <p className="text-[11px] text-slate-500">التحقق الآلي الفوري من هويات المزودين والعملاء عبر خدمة النفاذ الوطني الموحد.</p>
                  </div>
                </div>

                <div className="flex items-center gap-3">
                  <span className={`text-[10px] font-bold px-2.5 py-1 rounded-full border ${
                    platformData?.nafathEnabled 
                      ? 'bg-emerald-100 text-emerald-800 border-emerald-300' 
                      : 'bg-slate-200 text-slate-600 border-slate-300'
                  }`}>
                    {platformData?.nafathEnabled ? '✅ الخدمة مفعلة' : '⚪ معطلة'}
                  </span>

                  <label className="relative inline-flex items-center cursor-pointer">
                    <input
                      type="checkbox"
                      checked={!!platformData?.nafathEnabled}
                      onChange={(e) => {
                        updateSetting('nafathEnabled', e.target.checked);
                        showToast(e.target.checked ? 'تم تفعيل مفتاح الربط مع نفاذ الوطني الموحد بنجاح' : 'تم تعطيل الربط مع نفاذ', 'success');
                      }}
              ...
              {platformData?.nafathEnabled && (
                <div className="grid grid-cols-1 md:grid-cols-3 gap-4 pt-2 animate-in fade-in">
                  <div>
                    <label className="block text-xs font-bold text-slate-700 mb-1">بيئة الاتصال (Mode)</label>
                    <select
                      value={platformData?.nafathMode || 'sandbox'}
                      onChange={(e) => updateSetting('nafathMode', e.target.value)}
                      className="w-full p-2.5 rounded-xl border border-slate-200 text-xs font-bold bg-white focus:border-amber-500"
                    >
                      <option value="sandbox">🧪 بيئة الاختبار للربط المستقبلي (Nafath Sandbox)</option>
                      <option value="live">🚀 بيئة الإنتاج الحية المباشرة (Nafath Live)</option>
                    </select>
                  </div>
                  <div>
                    <label className="block text-xs font-bold text-slate-700 mb-1">App Client ID</label>
                    <input
                      type="text"
                      value={platformData?.nafathClientId || ''}
                      onChange={(e) => updateSetting('nafathClientId', e.target.value)}
                      placeholder="sa.gov.iam.lailah.client"
                      className="w-full p-2.5 rounded-xl border border-slate-200 text-xs font-mono text-left bg-white"
                      dir="ltr"
                    />
                  </div>
                  <div>
                    <label className="block text-xs font-bold text-slate-700 mb-1">API Service Secret / Key</label>
                    <input
                      type="password"
                      value={platformData?.nafathSecretKey || ''}
                      onChange={(e) => updateSetting('nafathSecretKey', e.target.value)}
                      placeholder="••••••••••••••••"
                      className="w-full p-2.5 rounded-xl border border-slate-200 text-xs font-mono text-left bg-white"
                      dir="ltr"
                    />
            ...
            <div className="bg-slate-50 border border-slate-200 rounded-2xl p-6 space-y-5">
              <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 border-b border-slate-200 pb-4">
                <div className="flex items-center gap-3">
                  <div className="w-10 h-10 rounded-xl bg-blue-600 text-white font-black flex items-center justify-center text-xs shadow-sm">
                    واثق
                  </div>
                  <div>
                    <h5 className="font-extrabold text-slate-800 text-sm">منصة واثق للتحقق من السجلات التجارية (Wathq CR Validation API)</h5>
                    <p className="text-[11px] text-slate-500">الفحص الفوري الآلي لسريان السجل التجاري والتراخيص للمنشآت والمزودين عند الانضمام.</p>
                  </div>
                </div>

                <div className="flex items-center gap-3">
                  <span className={`text-[10px] font-bold px-2.5 py-1 rounded-full border ${
                    platformData?.wathqEnabled 
                      ? 'bg-blue-100 text-blue-800 border-blue-300' 
                      : 'bg-slate-200 text-slate-600 border-slate-300'
                  }`}>
                    {platformData?.wathqEnabled ? '✅ الخدمة مفعلة' : '⚪ معطلة'}
                  </span>

                  <label className="relative inline-flex items-center cursor-pointer">
                    <input
                      type="checkbox"
                      checked={!!platformData?.wathqEnabled}
                      onChange={(e) => {
                        updateSetting('wathqEnabled', e.target.checked);
                        showToast(e.target.checked ? 'تم تفعيل التحقق الآلي عبر واثق بنجاح' : 'تم تعطيل خدمة واثق', 'success');
                      }}
                      className="sr-only peer"
                    />
              ...
              {platformData?.wathqEnabled && (
                <div className="grid grid-cols-1 md:grid-cols-3 gap-4 pt-2 animate-in fade-in">
                  <div>
                    <label className="block text-xs font-bold text-slate-700 mb-1">بيئة الربط (Sandbox / Production)</label>
                    <select
                      value={platformData?.wathqMode || 'sandbox'}
                      onChange={(e) => updateSetting('wathqMode', e.target.value)}
                      className="w-full p-2.5 rounded-xl border border-slate-200 text-xs font-bold bg-white focus:border-amber-500"
                    >
                      <option value="sandbox">🧪 بيئة الاختبار التجريبية (Wathq Sandbox)</option>
                      <option value="live">🚀 بيئة الإنتاج المباشرة (Wathq Production)</option>
                    </select>
                  </div>
                  <div>
                    <label className="block text-xs font-bold text-slate-700 mb-1">Wathq API Key</label>
                    <input
                      type="text"
                      value={platformData?.wathqApiKey || ''}
                      onChange={(e) => updateSetting('wathqApiKey', e.target.value)}
                      placeholder="wathq_live_key_xxx"
                      className="w-full p-2.5 rounded-xl border border-slate-200 text-xs font-mono text-left bg-white"
                      dir="ltr"
                    />
                  </div>
                  <div>
                    <label className="block text-xs font-bold text-slate-700 mb-1">إشعار الفحص التلقائي</label>
                    <div className="p-2 bg-blue-50 border border-blue-200 rounded-xl text-[11px] text-blue-800 font-bold mt-0.5">
                      يتم مطابقة رقم السجل التجاري مع وزارة التجارة تلقائياً فور الحفظ.
                    </div>
                  </div>
                </div>
              )}
            ...
            <div className="bg-slate-50 border border-slate-200 rounded-2xl p-6 space-y-5">
              <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 border-b border-slate-200 pb-4">
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    <h5 className="font-extrabold text-slate-800 text-sm">نظام شموس الأمني الموحد (Shamous Security Integration API)</h5>
                    <p className="text-[11px] text-slate-500">الربط والرفع التلقائي لبيانات النزلاء وحجوزات القاعات والمرافق للنظام الأمني الموحد.</p>
                  </div>
                </div>

                <div className="flex items-center gap-3">
                  <span className={`text-[10px] font-bold px-2.5 py-1 rounded-full border ${
                    platformData?.shamousEnabled 
                      ? 'bg-purple-100 text-purple-800 border-purple-300' 
                      : 'bg-slate-200 text-slate-600 border-slate-300'
                  }`}>
                    {platformData?.shamousEnabled ? '✅ الخدمة مفعلة' : '⚪ معطلة'}
                  </span>

                  <label className="relative inline-flex items-center cursor-pointer">
                    <input
                      type="checkbox"
                      checked={!!platformData?.shamousEnabled}
                      onChange={(e) => {
                        updateSetting('shamousEnabled', e.target.checked);
                        showToast(e.target.checked ? 'تم تفعيل مفتاح الربط مع نظام شموس الأمني بنجاح' : 'تم تعطيل الربط مع نظام شموس', 'success');
                      }}
                      className="sr-only peer"
                    />
              ...
              {platformData?.shamousEnabled && (
                <div className="grid grid-cols-1 md:grid-cols-3 gap-4 pt-2 animate-in fade-in">
                  <div>
                    <label className="block text-xs font-bold text-slate-700 mb-1">رمز المنشأة بنظام شموس (Establishment Code)</label>
                    <input
                      type="text"
                      value={platformData?.shamousEstablishmentCode || ''}
                      onChange={(e) => updateSetting('shamousEstablishmentCode', e.target.value)}
                  ...
                  <div>
                    <label className="block text-xs font-bold text-slate-700 mb-1">Shamous Gateway Token</label>
                    <input
                      type="password"
                      value={platformData?.shamousToken || ''}
                      onChange={(e) => updateSetting('shamousToken', e.target.value)}
                      placeholder="••••••••••••••••"
                      className="w-full p-2.5 rounded-xl border border-slate-200 text-xs font-mono text-left bg-white"
                      dir="ltr"
                    />
                  ...
                  <div>
                    <label className="block text-xs font-bold text-slate-700 mb-1">الإرسال التلقائي</label>
                    <select
                      value={platformData?.shamousAutoSync ? 'true' : 'false'}
                      onChange={(e) => updateSetting('shamousAutoSync', e.target.value === 'true')}
                      className="w-full p-2.5 rounded-xl border border-slate-200 text-xs font-bold bg-white focus:border-amber-500"
                    >
                      <option value="true">⚡ إرسال تلقائي لحظي فور مؤكدية الحجز</option>
                      <option value="false">📝 مراجعة يدوية قبل رفع سجل النزيل لشموس</option>
                    </select>
                  </div>
                </div>
              )}
