...
import { FileText, Shield, Sparkles, CheckCircle2, Clock, AlertCircle } from 'lucide-react';
...
export default function TermsPage() {
  ...
  useEffect(() => {
    fetch('/api/legal/content/terms_and_conditions')
      .then(res => res.json())
      .then(data => {
      ...
      .catch(err => {
        console.warn('Failed to load terms document:', err);
      })
      ...
      <div className="bg-gradient-to-b from-blue-950 to-slate-900 text-white py-16 px-4">
        <div className="max-w-4xl mx-auto space-y-4">
          <div className="inline-flex items-center gap-2 px-3 py-1 rounded-full bg-amber-500/20 text-amber-300 text-xs font-bold border border-amber-500/30">
            ...
            <span>الوثائق التعاقدية الرسمية</span>
            {doc?.version && (
              <span className="font-mono bg-amber-500 text-slate-950 px-2 py-0.5 rounded-full text-[10px]">
                v{doc.version}
              </span>
            )}
          </div>
          ...
          <h1 className="text-3xl sm:text-4xl font-black tracking-tight text-white">
            {doc?.title || 'الشروط والأحكام العامة لمنصة ليلة'}
          </h1>
          
          <p className="text-sm sm:text-base text-slate-300 max-w-2xl leading-relaxed">
            {doc?.subtitle || 'وثيقة الشروط والسياسات التعاقدية المعتمدة لحجز القاعات وإدارة الخدمات المساندة'}
          </p>
        ...
        <div className="bg-white rounded-3xl p-6 sm:p-10 md:p-12 shadow-sm border border-slate-200/80 space-y-8">
          ...
          {loading ? (
            <div className="py-20 text-center space-y-3">
              ...
              <p className="text-xs font-bold text-slate-500">جاري تحميل الشروط والأحكام الرسمية...</p>
            </div>
          ) : (
            <>
              {/* Intro Text */}
              {doc?.introText && (
                <div className="bg-amber-50/60 border border-amber-200/80 p-5 sm:p-6 rounded-2xl text-slate-800 text-sm sm:text-base leading-relaxed">
                  <p>{doc.introText}</p>
                </div>
              )}
              ...
              {sections.length > 0 ? (
                <div className="space-y-8 divide-y divide-slate-100">
                  {sections.map((sec, idx) => (
                    <div key={sec.id || idx} className={idx > 0 ? 'pt-8' : ''}>
                      <h2 className="text-xl sm:text-2xl font-bold text-blue-950 mb-4 border-r-4 border-amber-500 pr-3">
                        {sec.title}
                      </h2>
                ...
                <div className="text-slate-700 leading-relaxed text-sm sm:text-base whitespace-pre-wrap">
                  {doc?.contentAr || 'مرحباً بك في منصة وتطبيق ليلة. تنظم هذه الشروط والأحكام استخدامك للمنصة وكافة المعاملات وعقود الحجز والخدمات المبرمة من خلالها.'}
                </div>
              )}
